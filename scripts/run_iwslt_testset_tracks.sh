#!/usr/bin/env bash
# Run a maintained runtime preset on the blind IWSLT test-set
# for all three historical directions, back to back on a single GPU.
#
...
PRESET="${PRESET:-gemma_low_latency}"
...
preset = get_runtime_preset(os.environ["PRESET_NAME"])
...
for TGT in de it zh; do
  ...
  "$PYTHON_BIN" run_runtime_preset.py batch \
    --preset "$PRESET" \
    --source en \
    --target "$TGT" \
    --input-dir "$INPUT_DIR" \
