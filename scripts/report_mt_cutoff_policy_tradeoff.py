...
def policy_label(row: dict[str, Any]) -> str:
    if row.get("policy") == "alignatt":
        ...
        if tag == "alignatt" or tag == "alignatt_b1_top8":
            return "\\textsc{AlignAtt} submitted"
...
def _submitted_alignatt_row(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
...
def _selected_table_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    submitted = _submitted_alignatt_row(ordered)
    ...
    if pareto_alignatt:
        ...
        if submitted is None or tuned.get("tag") != submitted.get("tag"):
...
def build_json_payload(
    ...
    *,
    ...
    ordered = sorted(rows, key=sort_key)
    alignatt = _submitted_alignatt_row(ordered)
...
def main() -> None:
    ...
    frontier_tex_path = args.paper_generated_dir / args.frontier_tex_name
