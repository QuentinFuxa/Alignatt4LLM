...
- Presets: submission low/high regimes from ``cascade/submission.py``

Only the MT head set changes. Baseline top-8 metrics are read from the tracked
``submission/dev_logs`` bundles unless ``--baseline-root`` is overridden.
...
DEFAULT_REGIMES = ("low",)
DEFAULT_BASELINE_ROOT = REPO_ROOT / "submission" / "dev_logs"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("low", "high"),
        help="Submission latency regimes to evaluate.",
    )
    parser.add_argument(
        "--input-dir",
    ...
    parser.add_argument(
        ...
        help="Tracked baseline bundles with top-8 evaluation.json files.",
    )
    parser.add_argument(
        "--smoke-wav",
...
def build_runtime_config(
    *,
    ...
    source_code, target_code = direction.split("-", 1)
    preset_name = "main_low_latency" if regime == "low" else "main_high_latency"
