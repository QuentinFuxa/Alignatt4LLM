...
uses for text-to-text AlignAtt (see ``cascade.mt.base``): a fixed
...
class GemmaTransformersASRBackend(AlignmentBackend):
    ...
    def calibrate_alignment_heads(
        ...
        *,
        ...
        this is used only for head ranking — not as ground truth.
        """
        if self.model is None:
            raise RuntimeError("Gemma alignment backend is not loaded.")
