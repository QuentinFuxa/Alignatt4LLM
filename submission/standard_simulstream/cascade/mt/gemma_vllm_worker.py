...
targets a different capture function. In the shipped runtime, ``qwen_forced``
ASR is paired with ``gemma_vllm_alignatt`` MT, so only one of the two
Gemma-side workers is ever loaded in a given process.
"""
