...
backbone, and MT-side AlignAtt realization.
...
with a two-stage offline calibration procedure under the exact prompt layout
above: GPT-5-mini first provides word-level source-target alignments on held-out
parallel text, then every Gemma head is scored with Translation Score (TS) on
those aligned examples. We retain the top $k=8$ heads per language pair. This
head set is fixed at inference time and is the only part of the policy that
depends on offline calibration. Appendix~\ref{sec:appendix-alignatt} reports
the calibration details, corpus sizes, and per-direction score summaries. The
retained MT heads are sparse and concentrated in a limited late-depth region of
the backbone rather than spread uniformly; Appendix~\ref{sec:appendix-alignatt}
visualizes that pattern in Figure~\ref{fig:mt-alignatt-heads}.
...
structural rather than implementation noise. On the 78 French qualitative
probes used for our detailed diagnostics, drafted units allocate on average
9\% to accessible source tokens, 8\% to inaccessible source tokens, 81\% to
...
iff all three conditions above pass; a left-to-right first-failure scan then
emits the longest accepting draft prefix and rounds back to the nearest
stability-unit boundary so partial subword fragments are never emitted.
...
observer records prompt keys and draft-row queries for the selected heads while
returning $\Phi \equiv \mathbf{0}$ back into the attention output. After the
forward, a short Python-side replay reconstructs only the selective
draft-to-source block consumed by the policy and by the parity suite. The
sentinel term is therefore only a liveness device: without the additive zero,
graph lowering dead-code-eliminates the observer path and no usable Q/K
buffers survive on the deployed runtime. Appendix~\ref{sec:appendix-observer}
keeps the replay equations, parity measurements, and qualitative probe.
...
clearly stronger quality point at the expected latency cost. We therefore read
...
This appendix keeps only the details omitted from
Section~\ref{sec:method}: the prompt/suffix replay equations, one qualitative
probe, and the numerical parity measurements on the engine-native path.
...
\mathbf{B}^{(\ell)}_\star[h]$ for head-specific captured tensors, we compute
\begin{align}
  \mathbf{P}^{(\ell,h)} &=
...
stack on a text-only MT probe: the exact prompt partition seen by the decoder,
word-level aggregation of the replayed rows for readability, and the four-way
provenance accounting for every drafted word.
