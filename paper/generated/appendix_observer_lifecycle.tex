...
\begin{tikzpicture}[
  ...
  bufnode/.style={draw=teal!60!black, fill=teal!14, rounded corners=2pt,
                  line width=0.7pt, inner sep=3pt, font=\scriptsize,
                  align=center},
  outnode/.style={draw=black!55, fill=black!3, rounded corners=2pt,
                  line width=0.7pt, inner sep=3pt, font=\scriptsize,
                  align=center},
  tagbox/.style={draw=black!45, fill=black!2, rounded corners=2pt,
  ...
  \node[sectitle, text=black!85, anchor=north west] at (0.20,7.65)
    {A.~Setup: install observer before graph capture};
  \node[sub, anchor=north west, text width=7.80cm] at (0.20,7.30)
    {runs once, before the deployed graph is fixed};
  ...
  \node[prose, anchor=north west, text width=7.60cm] at (0.20,1.10)
    {\emph{Why this path is retained}: fixed shapes let AOT resolve every
  ...
  \node[sub, anchor=north west, text width=8.40cm] at (8.40,7.30)
    {observer ops run inside the deployed graph; replay runs in Python on the
     stored slots};
  ...
  % Edges -- L-shape elbows so arrows enter the west edge of the softmax box
  % cleanly, away from the top/bottom corners.
  \draw[dflow] (bK.east) -| ([xshift=-0.40cm, yshift=0.35cm]replay.west)
               -- ([yshift=0.35cm]replay.west);
  \draw[dflow] (bQ.east) -| ([xshift=-0.40cm, yshift=-0.35cm]replay.west)
               -- ([yshift=-0.35cm]replay.west);
...
\caption{\textbf{Observer lifecycle on the deployed vLLM path.}
...
in the live fan-in of the graph so graph optimization cannot dead-code-eliminate
it, while the numerical output of the layer is unchanged.
\emph{Right (B), every forward:} after the forward finishes, the
...
the policy, the parity check against a Transformers reference,
