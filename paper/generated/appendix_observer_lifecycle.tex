...
\begin{tikzpicture}[
  ...
  \node[outnode, text width=3.00cm, align=center, minimum height=1.40cm]
    ...
    {$\mathrm{softmax}(\mathbf{Q}\mathbf{K}^{\!\top}\!+\mathbf{m})$};
  ...
  \node[tagbox, anchor=north west, text width=3.85cm] at (8.40,2.85)
    {policy features (Fig.~\ref{fig:appendix-policy-pipeline})};
...
Python-side replay computes $\mathbf{Q}\mathbf{K}^{\!\top}$ for the draft
against the full prompt, applies the causal mask $\mathbf{m}$, takes the
softmax, and keeps the source columns to yield the selective block
...
\label{fig:appendix-observer-lifecycle}
\end{figure*}
