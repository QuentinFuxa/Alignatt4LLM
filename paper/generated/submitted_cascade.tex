...
\begin{tikzpicture}[
  ...
  asrbox/.style    ={box, draw=green!45!black, fill=green!5},
  mtbox/.style     ={box, draw=blue!50!black,  fill=blue!4},
  obsbox/.style    ={box, draw=orange!75!black, fill=orange!7,
                     line width=1.0pt},
  outbox/.style    ={box, draw=teal!60!black,  fill=teal!6},
  ...
  feed/.style      ={->, line width=0.8pt, draw=teal!55!black, dashed},
  ...
  \node[hdr, text=green!30!black, anchor=north] at ({(\bL+\bR)/2},\bYT-0.18)
    {Qwen3 forced ASR};
  ...
  % Local Agreement self-loop on Qwen3 ASR box
  \draw[->, draw=green!45!black, line width=0.75pt, rounded corners=4pt]
    ({\bR-0.20},\bYT+0.02)
    -- ({\bR-0.20},\bYT+0.45)
    -- ({\bL+0.20},\bYT+0.45)
    -- ({\bL+0.20},\bYT+0.02);
  \node[font=\tiny, text=green!30!black, anchor=south]
    at ({(\bL+\bR)/2},\bYT+0.48)
    {Local Agreement on prefix};

  %% ---- Stage 3: MT prompt with explicit layout ----
  \filldraw[mtbox] (\cL,\bYB) rectangle (\cR,\bYT);
  \node[hdr, text=blue!40!black, anchor=north] at ({(\cL+\cR)/2},\bYT-0.18)
    {Gemma-4 MT prompt};
  ...
  %% ---- Stage 4: Engine-native AlignAtt observer (HERO) ----
  \filldraw[obsbox] (\dL,\bYB) rectangle (\dR,\bYT);
  \node[hdr, text=orange!55!black, anchor=north] at ({(\dL+\dR)/2},\bYT-0.18)
    {Engine-native AlignAtt observer};
  \node[sub, anchor=north, font=\tiny\itshape, text=orange!55!black]
    at ({(\dL+\dR)/2},\bYT-0.52)
    {main contribution of this paper};
  % Mini attention matrix (3 rows draft x 7 cols source)
  \def\matL{9.60}\def\matB{1.85}
  \def\cellw{0.19}
  \foreach \r/\c/\k in {
  ...
  \node[tagL, anchor=west] at (\hintX,3.11)
    {\textbullet\ \ compile-safe $q/k$ capture};
  ...
  \node[hdr, text=teal!45!black, anchor=north] at ({(\eL+\eR)/2},\bYT-0.18)
    {committed target};
  ...
  \draw[feed, rounded corners=4pt]
    ...
    -- (\ypSlotCx,\fbY)
    -- (\ypSlotCx,\bYB-0.03);
  \node[font=\tiny\itshape, text=teal!45!black, anchor=south]
...
decoder-only causal layout, so the engine-native observer (\emph{main
contribution of this paper}) can capture the selected heads' queries and keys
on the deployed vLLM path and reconstruct only the draft-to-source block
