...
\begin{tikzpicture}[
  ...
  \def\Wmax{17.00}\def\Hmax{6.40}
  \draw[panel] (0.00,0.00) rectangle (\Wmax,\Hmax);

  % === Input: reconstructed tensor block ===================================
  \def\ixL{0.55}\def\ixR{2.25}\def\iyB{1.90}\def\iyT{5.30}

  \node[sectitle, text=black!80, anchor=south, text width=3.0cm, align=center]
    at ({(\ixL+\ixR)/2},\iyT+0.40)
  ...
  \draw[black!75, line width=0.85pt, -] (\jx,{(\iyB+\iyT)/2}) -- (\jx,4.90);
  \draw[black!75, line width=0.85pt, -] (\jx,{(\iyB+\iyT)/2}) -- (\jx,2.30);
  \draw[flow] (\jx,4.90) -- (3.50,4.90);
  \draw[flow] (\jx,2.30) -- (3.50,2.30);

  % === Branch A: Accessible-source mass ====================================
  \def\aL{3.55}\def\aR{10.70}\def\aB{3.80}\def\aT{5.85}
  ...
  \def\bL{3.55}\def\bR{10.70}\def\bB{0.75}\def\bT{2.90}
  ...
  % === Gate ================================================================
  \def\gL{11.30}\def\gR{14.35}\def\gB{0.90}\def\gT{5.85}
  ...
  \node[eqn, anchor=west] at (\gL+0.20,\gT-1.30)
    {(i)\ \ $\hat s_t\,\in\,\mathcal{S}^{(k)}_{\mathrm{acc}}$};
  \node[sublab, anchor=west, text width=2.50cm]
    at (\gL+0.40,\gT-1.75) {peak stays left of the frontier};

  \node[eqn, anchor=west] at (\gL+0.20,\gT-2.45)
    {(ii)\ \ $p_{t,\hat s_t}\,\ge\,\tau_{\mathrm{peak}}$};
  \node[sublab, anchor=west, text width=2.50cm]
    at (\gL+0.40,\gT-2.90) {local peak mass high enough};

  \node[eqn, anchor=west] at (\gL+0.20,\gT-3.60)
    {(iii)\ \ $\pi^{\mathrm{acc}}_t\,\ge\,\tau_{\mathrm{prov}}$};
  \node[sublab, anchor=west, text width=2.50cm]
    at (\gL+0.40,\gT-4.05) {enough accessible mass overall};
  ...
  \node[sectitle, anchor=north west] at (\sL,5.80) {Scan};
  \node[sublab, anchor=north west, text width=1.85cm] at (\sL,5.45)
...
$\tau_{\mathrm{peak}}$, and the accessible-source mass exceeds
$\tau_{\mathrm{prov}}$. A left-to-right scan emits the longest run of
accepting tokens and drops the remainder at the first failure.}
\label{fig:appendix-policy-pipeline}
\end{figure*}
