#!/usr/bin/env python3
"""Summarize EN->ZH source-frontier hard-stop recovery opportunities."""
...
from tools.reports.report_enzh_source_regression_diagnostics import (
    finite_float,
    finite_int,
    int_positions,
    iter_stream_updates,
...
REPORT_COLUMNS = (
    ...
    "longyaal_cu_ms",
    "xcometxl",
    "source_frontier_stop_count",
    "source_frontier_stop_ratio",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-stem",
        default="enzh_source_frontier_diagnostics_20260607",
    )
    return parser.parse_args()


def source_frontier_blocks(
    *,
    position: int | None,
    ...
    if int(position) < max(0, accessible_count) + border_margin:
        return False
...
def token_passes_other_gates(
    *,
    ...
    return token_passes_non_regression_gates(
        ...
        include_source_frontier_gate=False,
        include_token_argmax_frontier_gate=True,
    )


def recoverable_source_frontier_accept_count(
    ...
    *,
    ...
    for token_index in range(max(0, accepted_count), len(positions)):
        ...
        if not token_passes_other_gates(
            position=position,
            provenance=row,
            metadata=metadata,
            runtime_config=runtime_config,
...
def analyze_run(index_row: dict[str, str]) -> dict[str, Any] | None:
    ...
    for update in iter_stream_updates(copied_dir / "stream_updates.jsonl") or []:
        ...
        if metadata.get("stop_reason") != "alignatt:source_frontier":
            continue
        stop_count += 1
    ...
    return {
        ...
        "source_frontier_stop_count": stop_count,
        "source_frontier_stop_ratio": (
        ...
        "translation_alignatt_border_margin": finite_int(
            runtime_config.get("translation_alignatt_border_margin"),
            default=0,
        ),
        "translation_alignatt_frontier_min_inaccessible_mass": (
...
def report_rows(index_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    rows.sort(
        key=lambda row: (
            -int(row["source_frontier_stop_count"]),
...
def write_outputs(rows: list[dict[str, Any]], output_dir: Path, stem: str) -> None:
    ...
    summary = {
        "run_count": len(rows),
        "runs_with_source_frontier_stops": sum(
            1 for row in rows if int(row["source_frontier_stop_count"]) > 0
        ),
        "total_source_frontier_stops": sum(
            int(row["source_frontier_stop_count"]) for row in rows
        ),
