...
Each sub-job materialises the same artifacts as ``alignatt-batch``:
`manifest.json`, `hypothesis.jsonl`, `stream_updates.jsonl`. The scripted
preset is equivalent to `gemma_low_latency` / `gemma_high_latency` except for
`chunk_ms`.

Usage (from `.venv-inference`):

    python tools/research/run_additive_chunk_sweep.py \
        --chunk-ms 850 \
...
from alignatt4llm.runtime import LoadedModelBundle
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-preset",
        default="gemma_low_latency",
        help=(
            "Runtime preset whose knobs seed the config (only "
            "chunk_ms is overridden). Default: gemma_low_latency."
        ),
    )
    parser.add_argument(
        "--directions",
