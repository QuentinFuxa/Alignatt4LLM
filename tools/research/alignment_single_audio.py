#!/usr/bin/env python
"""Single-audio diagnostic harness for the Gemma AlignAtt ASR research path.

Retained commands:

- ``baseline`` produces the Qwen teacher bundle for a clip and writes it
  as a JSON artifact.
- ``gemma_vllm_inspect`` runs the Gemma vLLM AlignAtt backend
  (``gemma_vllm_qk_fast``) on the same clip and writes its transcript +
  word timestamps + per-token audio positions.
- ``seam_comparison`` runs the three-seam vLLM comparison on one audio:
  eager baseline, cudagraph=full, vllm_compile+cudagraph=none.
- ``compare`` loads two bundles and prints a word-end-MAE table.
...
from alignatt4llm.alignment.base import (
...
def _write_repeat_stability_summary(
    ...
    *,
    ...
    """Report text stability, token-count drift, and timing variance for a repeat run."""
...
def build_cli() -> argparse.ArgumentParser:
    ...
    vllm_inspect = subparsers.add_parser(
        "gemma_vllm_inspect",
        help="Run Gemma ASR + AlignAtt through vLLM (gemma_vllm_qk_fast) on one clip.",
    )
