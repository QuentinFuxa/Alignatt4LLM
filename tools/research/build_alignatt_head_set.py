...
``translation_heads_*.json`` file that the AlignAtt MT backend can consume
directly. Pair it with ``alignatt-batch --target ...`` and a runtime override
of ``translation_alignatt_heads_path`` to drive head-set comparison sweeps once
a GPU is available.
