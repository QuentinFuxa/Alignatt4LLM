...
| File | Role |
|---|---|
| `paper_artifact.py` | **Canonical PDF → JSON extractor.** Produces a schema-versioned `PaperArtifact` (title / authors / abstract / paragraph chunks). Deterministic; no LLM. CLI: `python -m cascade.paper_context.paper_artifact <pdf>...`. |
...
See [`../../docs/system.md`](../../docs/system.md) for the mechanism
design and the current runtime contract.
