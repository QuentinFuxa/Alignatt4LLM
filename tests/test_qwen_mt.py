"""Tests for the Qwen2.5 reference MT backend and its observer wiring.
...
def test_runtime_registration_resolves_model_and_heads_path():
    ...
    assert "Qwen2.5" in mt_model_name_for_backend("qwen_vllm_alignatt")
    path = alignatt_heads_path_for(
        "English", "German", mt_backend_name="qwen_vllm_alignatt"
    )
    assert path.endswith(
        "translation_heads_Qwen_Qwen2_5-1_5B-Instruct_en-de.json"
    )


def test_qwen_spec_targets_qwen2_attention_with_standard_forward():
    pytest.importorskip("torch")
    ...
    assert QWEN_SPEC.family == "qwen2"
    assert QWEN_SPEC.attention_import_paths == (
        ("vllm.model_executor.models.qwen2", "Qwen2Attention"),
    )
    # Reference uses the standard (no-QK-norm) forward; building it must work.
...
def test_qwen2_attention_patch_is_installable():
    # Only runs where vLLM's Qwen2 attention class is importable (GPU/inference
    # env). Mirrors test_milmmt_mt's Gemma patch-installability check.
    pytest.importorskip("vllm.model_executor.models.qwen2")
    from alignatt4llm.mt.qwen_vllm_backend import QWEN_SPEC
    ...
    assert hasattr(Qwen2Attention, "_alignatt_qwen2_mt_qk_original_forward")
