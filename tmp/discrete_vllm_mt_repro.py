#!/usr/bin/env python3
"""Re-run discrete vLLM MT to measure run-to-run reproducibility.

Same config as tmp/discrete_vllm_mt.py, different output dir.
Purpose: establish the vLLM-MT non-determinism floor so we can
isolate the scalar-vs-discrete divergence from scheduler noise.
"""
...
def main():
    ...
    print("Loading models (discrete + vLLM MT, REPRO run) ...", flush=True)
    ...
    print(f"\n==== discrete_vllm_mt_repro ====", flush=True)
