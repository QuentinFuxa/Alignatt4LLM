#!/usr/bin/env python3
"""3-way context A/B on clip 3 myfXyntFYL (Prompting PaLM paper)."""
...
def run_at_mode(mode: str, reset: bool) -> None:
    ...
    if mode != "off":
        cfg.paper_context_path = "data/paper_artifacts/myfXyntFYL.json"
        cfg.paper_context_mode = mode
        cfg.paper_context_top_k = 3
        cfg.paper_context_max_chars = 1200

    if reset:
        print(f"Loading models (clip3 context mode={mode})...", flush=True)
    ...
    print(f"\n==== context_clip3_{mode} ====", flush=True)
    batch_start = perf_counter()
    result = run_single_audio(
        processor, "test-set/audio/myfXyntFYL.wav", 450, "de", "en",
    )
    batch_wallclock_s = perf_counter() - batch_start
    print(f"  mode={mode} RTF={result['rtf']:.3f} updates={result['num_updates']} wallclock={batch_wallclock_s:.1f}s", flush=True)
    output_dir = f"outputs/night2_context_clip3_{mode}"
