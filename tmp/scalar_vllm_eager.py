#!/usr/bin/env python3
"""Scalar substitution A/B with vLLM MT in enforce_eager mode.

The compile-cache fragility blocks AOT-compiled vLLM MT runs
consistently (see DECISIONS). enforce_eager=True skips AOT compile
entirely, running inference through PyTorch eager mode. This
sidesteps the ValueError while still exercising the scalar
substitution on the vLLM MT backend (albeit slower than a compiled
vLLM MT).
"""
...
def main():
    ...
    print("Loading models (scalar + vLLM MT, enforce_eager) ...", flush=True)
    ...
    output_dir = "outputs/night1_ende_scalar_vllm_eager_instrumented"
