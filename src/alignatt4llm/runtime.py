...
from alignatt4llm.alignment.base import AlignmentBackend, AlignmentResult, WordAlignment
from alignatt4llm.alignment.gemma_alignatt_stream import (
    GemmaAlignAttStream,
    StreamStepDelta,
)
...
from alignatt4llm.mt.base import MTBackendResult, PromptCacheState, build_mt_backend
from alignatt4llm.source_frontier import (
    SourceAccessibilityFrontier,
    build_source_accessibility_frontier,
    iter_source_word_spans,
...
from alignatt4llm.paper_context import (
    CONTEXT_MODE_OFF,
    VALID_CONTEXT_MODES,
    PaperArtifact,
    PaperContextBlock,
...
VALID_MILMMT_PROMPT_MODES = ("direct",)
VALID_ALIGNATT_ONLINE_NORMALIZATIONS = ("zscore", "raw")
...
VALID_ALIGNATT_SOURCE_CONTEXT_MODES = ("block", "target_unit_cap")
VALID_ALIGNATT_SOURCE_FRONTIER_ACTIONS = (
    "stop",
    "trim_unrecovered",
)
VALID_ALIGNATT_SOURCE_REGRESSION_REFERENCE_MODES = ("max", "median_recent")
...
class CascadeRuntimeConfig:
    ...
    translation_alignatt_acceptance_variant: str = "token"
    translation_alignatt_online_normalization: str = "zscore"
    ...
    translation_alignatt_source_frontier_action: str = "stop"
    # Provenance confidence gates. Defaults keep AlignAtt permissive; caps
    # below 1.0 are explicit guarded diagnostics.
    translation_alignatt_max_inaccessible_source_mass: float = 1.0
    translation_alignatt_max_non_source_prompt_mass: float = 1.0
    translation_alignatt_min_accessible_inaccessible_margin: float = -1.0
    translation_alignatt_min_accepted_accessible_source_mass: float = 0.0
    translation_alignatt_accepted_accessible_source_mass_recent_units: int = 2
    ...
    translation_alignatt_min_accessible_source_units: int = 0
    translation_alignatt_min_accessible_source_units_mode: str = "block"
    translation_alignatt_hold_back_target_units: int = 0
    translation_alignatt_min_emit_target_units: int = 0
    translation_alignatt_max_source_regression: int = -1
    translation_alignatt_source_regression_min_source_mass: float = 0.0
    translation_alignatt_source_regression_min_inaccessible_mass: float = 0.0
    translation_alignatt_source_regression_recent_tokens: int = 0
    translation_alignatt_source_regression_reference_mode: str = "max"
    translation_alignatt_source_regression_activation_mode: str = "always"
    translation_alignatt_source_regression_activation_slack_tokens: int = 0
    translation_alignatt_source_regression_patience_tokens: int = 1
    translation_alignatt_source_regression_action: str = "stop"
    translation_alignatt_unit_consensus_min_head_ratio: float = 0.60
    # Alignment-confidence floor for the `unit_conf` acceptance variant: the
    # minimum fraction of selected heads whose source argmax agrees (within
    # one token) with the consensus position. 0.0 disables the gate, making
    # `unit_conf` behave exactly like `unit_argmax`.
    translation_alignatt_min_alignment_confidence: float = 0.0
    ...
    translation_alignatt_token_argmax_frontier_gate: bool = False
    translation_alignatt_token_argmax_min_source_mass: float = 0.05
    translation_alignatt_token_argmax_frontier_margin: int = 0
    translation_alignatt_token_argmax_frontier_patience_tokens: int = 1
    translation_alignatt_source_lookback_holdback: bool = False
    translation_alignatt_source_lookback_units: int = 2
    translation_alignatt_source_lookback_min_source_mass: float = 0.05
    translation_alignatt_source_lookback_min_source_position: int = 3
    translation_alignatt_defer_low_source_terminal_punctuation: bool = False
    translation_alignatt_terminal_punctuation_min_source_mass: float = 0.06
    ...
    milmmt_prompt_mode: str = "direct"
    # Prepend the tokenizer BOS to the MiLMMT raw completion prompt. The
    # offline screening protocol that established MiLMMT's quality ceiling
    # tokenized prompts WITH special tokens (BOS), while the streaming path
    # historically tokenized with add_special_tokens=False; this knob closes
    # that gap. Default False keeps old runs byte-reproducible; flipping it
    # shifts every source-map token position by +1 (handled internally).
    milmmt_prompt_add_bos: bool = False
    milmmt_temperature: float = 0.0
    milmmt_top_p: float = 1.0
    milmmt_top_k: int = 1
    milmmt_repetition_penalty: float = 1.0
    # AlignAtt streaming ASR knobs (gemma_vllm_qk_fast only). See
    # ``alignatt4llm/alignment/gemma_alignatt_stream.py``. ``frontier_flush``
    ...
    # Qwen forced-aligner punctuation commits can split very short source
    # greetings before the MT target has enough context to avoid duplication
    # in languages such as Chinese. Default 0 preserves historical behavior.
    asr_punctuation_min_commit_words: int = 0
    # Self-conditioning for the qwen_forced ASR: pass the last N words of the
    # talk's already-committed transcript as the qwen_asr `context` prompt.
    # Internal conditioning on the system's own past ASR output; it does not
    # change the no-context comparison class (the public baseline's context
    # axis is TRANSLATION history). 0 disables it.
    asr_context_committed_words: int = 0
    ...
    # current-source span. See docs/system.md.
    ...
    def _validate(self) -> None:
        ...
        if int(self.asr_punctuation_min_commit_words) < 0:
            raise ValueError(
                "asr_punctuation_min_commit_words must be >= 0, got "
                f"{self.asr_punctuation_min_commit_words!r}."
            )
        if int(self.asr_context_committed_words) < 0:
            raise ValueError(
                "asr_context_committed_words must be >= 0, got "
                f"{self.asr_context_committed_words!r}."
            )
        ...
        if self.milmmt_prompt_mode not in VALID_MILMMT_PROMPT_MODES:
            raise ValueError(
                "milmmt_prompt_mode must be one of "
                f"{VALID_MILMMT_PROMPT_MODES}, got {self.milmmt_prompt_mode!r}."
            )
        if self.translation_alignatt_acceptance_variant not in VALID_ALIGNATT_ACCEPTANCE_VARIANTS:
            raise ValueError(
                "translation_alignatt_acceptance_variant must be one of "
                f"{VALID_ALIGNATT_ACCEPTANCE_VARIANTS}, got "
                f"{self.translation_alignatt_acceptance_variant!r}."
            )
        if self.translation_alignatt_online_normalization not in VALID_ALIGNATT_ONLINE_NORMALIZATIONS:
            raise ValueError(
                "translation_alignatt_online_normalization must be one of "
                f"{VALID_ALIGNATT_ONLINE_NORMALIZATIONS}, got "
                f"{self.translation_alignatt_online_normalization!r}."
            )
        ...
        bounded_unit_interval = {
            ...
            "translation_alignatt_max_non_source_prompt_mass": self.translation_alignatt_max_non_source_prompt_mass,
            "translation_alignatt_min_accepted_accessible_source_mass": self.translation_alignatt_min_accepted_accessible_source_mass,
            "translation_alignatt_argmax_mass_threshold": self.translation_alignatt_argmax_mass_threshold,
            "translation_alignatt_unit_consensus_min_head_ratio": self.translation_alignatt_unit_consensus_min_head_ratio,
            "translation_alignatt_min_alignment_confidence": self.translation_alignatt_min_alignment_confidence,
            ...
            "translation_alignatt_token_argmax_min_source_mass": self.translation_alignatt_token_argmax_min_source_mass,
            "translation_alignatt_source_lookback_min_source_mass": self.translation_alignatt_source_lookback_min_source_mass,
            "translation_alignatt_terminal_punctuation_min_source_mass": self.translation_alignatt_terminal_punctuation_min_source_mass,
            "translation_alignatt_source_regression_min_inaccessible_mass": self.translation_alignatt_source_regression_min_inaccessible_mass,
        }
        ...
        nonnegative_ints = {
            "translation_alignatt_min_accessible_source_units": self.translation_alignatt_min_accessible_source_units,
            "translation_alignatt_hold_back_target_units": self.translation_alignatt_hold_back_target_units,
            "translation_alignatt_min_emit_target_units": self.translation_alignatt_min_emit_target_units,
            "translation_alignatt_accepted_accessible_source_mass_recent_units": self.translation_alignatt_accepted_accessible_source_mass_recent_units,
            "translation_alignatt_token_argmax_frontier_margin": self.translation_alignatt_token_argmax_frontier_margin,
            "translation_alignatt_token_argmax_frontier_patience_tokens": self.translation_alignatt_token_argmax_frontier_patience_tokens,
            "translation_alignatt_source_lookback_units": self.translation_alignatt_source_lookback_units,
            "translation_alignatt_source_lookback_min_source_position": self.translation_alignatt_source_lookback_min_source_position,
            "translation_alignatt_source_regression_recent_tokens": self.translation_alignatt_source_regression_recent_tokens,
            "translation_alignatt_source_regression_activation_slack_tokens": self.translation_alignatt_source_regression_activation_slack_tokens,
            "translation_alignatt_source_lcp_append_slack_units": self.translation_alignatt_source_lcp_append_slack_units,
        }
        for name, value in nonnegative_ints.items():
            if int(value) < 0:
                raise ValueError(f"{name} must be >= 0, got {value!r}.")
        if (
            int(self.translation_alignatt_source_lcp_append_slack_units) > 0
            and not bool(self.translation_alignatt_source_lcp_stability)
        ):
            raise ValueError(
                "translation_alignatt_source_lcp_append_slack_units requires "
                "translation_alignatt_source_lcp_stability=True."
            )
        if int(self.translation_alignatt_source_regression_patience_tokens) < 1:
            raise ValueError(
                "translation_alignatt_source_regression_patience_tokens must be >= 1, got "
                f"{self.translation_alignatt_source_regression_patience_tokens!r}."
            )
        if int(self.translation_alignatt_token_argmax_frontier_patience_tokens) < 1:
            raise ValueError(
                "translation_alignatt_token_argmax_frontier_patience_tokens must be "
                ">= 1, got "
                f"{self.translation_alignatt_token_argmax_frontier_patience_tokens!r}."
            )
        if (
            self.translation_alignatt_source_frontier_action
            not in VALID_ALIGNATT_SOURCE_FRONTIER_ACTIONS
        ):
            valid = ", ".join(VALID_ALIGNATT_SOURCE_FRONTIER_ACTIONS)
            raise ValueError(
                "translation_alignatt_source_frontier_action must be "
                f"one of {valid}, got "
                f"{self.translation_alignatt_source_frontier_action!r}."
            )
        if (
            self.translation_alignatt_min_accessible_source_units_mode
            not in VALID_ALIGNATT_SOURCE_CONTEXT_MODES
        ):
            valid = ", ".join(VALID_ALIGNATT_SOURCE_CONTEXT_MODES)
            raise ValueError(
                "translation_alignatt_min_accessible_source_units_mode must be "
                f"one of {valid}, got "
                f"{self.translation_alignatt_min_accessible_source_units_mode!r}."
            )
        if int(self.translation_alignatt_max_source_regression) < -1:
            raise ValueError(
                "translation_alignatt_max_source_regression must be >= -1, got "
                f"{self.translation_alignatt_max_source_regression!r}."
            )
        if (
            self.translation_alignatt_source_regression_reference_mode
            not in VALID_ALIGNATT_SOURCE_REGRESSION_REFERENCE_MODES
        ):
            valid = ", ".join(VALID_ALIGNATT_SOURCE_REGRESSION_REFERENCE_MODES)
            raise ValueError(
                "translation_alignatt_source_regression_reference_mode must be "
                f"one of {valid}, got "
                f"{self.translation_alignatt_source_regression_reference_mode!r}."
            )
        if (
            self.translation_alignatt_source_regression_activation_mode
            not in VALID_ALIGNATT_SOURCE_REGRESSION_ACTIVATION_MODES
        ):
            valid = ", ".join(VALID_ALIGNATT_SOURCE_REGRESSION_ACTIVATION_MODES)
            raise ValueError(
                "translation_alignatt_source_regression_activation_mode must be "
                f"one of {valid}, got "
                f"{self.translation_alignatt_source_regression_activation_mode!r}."
            )
        if (
            self.translation_alignatt_source_regression_action
            not in VALID_ALIGNATT_SOURCE_REGRESSION_ACTIONS
        ):
            valid = ", ".join(VALID_ALIGNATT_SOURCE_REGRESSION_ACTIONS)
            raise ValueError(
                "translation_alignatt_source_regression_action must be "
                f"one of {valid}, got "
                f"{self.translation_alignatt_source_regression_action!r}."
            )
        if not 0.0 <= float(self.translation_alignatt_source_regression_min_source_mass) <= 1.0:
            raise ValueError(
                "translation_alignatt_source_regression_min_source_mass must be "
                "in [0, 1], got "
                f"{self.translation_alignatt_source_regression_min_source_mass!r}."
            )
...
def committed_transcript_tail(
    committed_segments: Sequence[str],
    *,
    max_words: int,
) -> str:
    """Last ``max_words`` words of the talk's committed transcript.

    Used as the qwen_asr ``context`` prompt when
    ``asr_context_committed_words`` is positive. Returns "" when disabled or
    ...
    for segment in committed_segments:
        words.extend(str(segment).split())
    if not words:
        return ""
...
def derive_monotone_partial_acceptance(
    *,
    ...
    candidate_generated_ids = tuple(
        int(token_id) for token_id in result.accepted_generated_token_ids
    )
    source_prefix = source_prefix.strip()
    if not source_prefix:
        return "", (), ()
    if not candidate_text:
        return (
            previous_state.accepted_target,
            previous_state.accepted_token_ids,
            previous_state.accepted_generated_token_ids,
        )
    if not previous_state.source_prefix:
        return candidate_text, candidate_ids, candidate_generated_ids
    ...
    if candidate_ids[: len(previous_accepted_ids)] != previous_accepted_ids:
        return (
            previous_state.accepted_target,
            previous_accepted_ids,
            previous_state.accepted_generated_token_ids,
        )
    return candidate_text, candidate_ids, candidate_generated_ids


def source_bounded_prefill_generated_token_ids(
    *,
    accepted_generated_token_ids: Sequence[int],
    aligned_source_unit_indices: Sequence[int | None],
    final_source_unit_count: int,
) -> tuple[int, ...]:
    """Keep accepted target tokens whose AlignAtt source is inside a final segment.

    A partial target prefix may cover more source than the punctuation/LCP ASR
    path has just committed as a final segment. Reusing that full target as an
    assistant prefill lets final-segment MT bypass the partial AlignAtt policy.
    This helper keeps only the monotone token prefix whose source-unit argmaxes
    are still within the newly final source segment.
    """

    final_ids, _ = split_source_bounded_prefill_generated_token_ids(
        accepted_generated_token_ids=accepted_generated_token_ids,
        aligned_source_unit_indices=aligned_source_unit_indices,
        final_source_unit_count=final_source_unit_count,
    )
    return final_ids


def split_source_bounded_prefill_generated_token_ids(
    *,
    accepted_generated_token_ids: Sequence[int],
    aligned_source_unit_indices: Sequence[int | None],
    final_source_unit_count: int,
) -> tuple[tuple[int, ...], tuple[int, ...]]:
    token_ids = tuple(int(token_id) for token_id in accepted_generated_token_ids)
    if not token_ids:
        return (), ()
    if int(final_source_unit_count) <= 0:
        return (), token_ids
    if not aligned_source_unit_indices:
        return token_ids, ()

    split_index = len(token_ids)
    for token_index, source_unit_index in enumerate(aligned_source_unit_indices):
        if token_index >= len(token_ids):
            break
        if source_unit_index is None:
            continue
        if int(source_unit_index) >= int(final_source_unit_count):
...
class TranslationUnitManager:
    ...
    def build_source_frontier(
        ...
        *,
        ...
        return build_source_accessibility_frontier(
            ...
            is_final=is_final,
            max_accessible_chars=self.source_lcp_stable_prefix_chars(source_text.text)
            if not is_final
            else None,
        )

    def source_lcp_stable_prefix_chars(self, source_text: str) -> int | None:
        if not bool(self.config.translation_alignatt_source_lcp_stability):
            return None
        hypotheses = self.state.asr_hypotheses
        if len(hypotheses) < 2:
            return 0
        previous = normalize_source_text_for_mt(
            normalize_partial_asr_hypothesis(hypotheses[-2])
        ).text
        current = normalize_source_text_for_mt(source_text).text
        if not previous or not current:
            return 0
        stable_chars = len(longest_common_prefix(previous, current))
        if not current.startswith(previous):
            return stable_chars
        if len(current) > len(previous) and not current[len(previous)].isspace():
            return stable_chars
        return self.source_lcp_prefix_chars_with_append_slack(
            current,
            stable_chars=stable_chars,
        )

    def source_lcp_prefix_chars_with_append_slack(
        self,
        source_text: str,
        *,
        stable_chars: int,
    ) -> int:
        append_slack_units = int(
            self.config.translation_alignatt_source_lcp_append_slack_units
        )
        if append_slack_units <= 0:
            return max(0, int(stable_chars))
        char_cap = max(0, int(stable_chars))
        slack_used = 0
        for _, char_end, _ in iter_source_word_spans(source_text):
            if int(char_end) <= char_cap:
                continue
            char_cap = int(char_end)
            slack_used += 1
            if slack_used >= append_slack_units:
                break
        return char_cap
    ...
    def snapshot_skipped_partial_result(
        ...
        *,
        ...
        alignatt_metadata.update(
            {
                ...
                "source_lcp_stability_enabled": bool(
                    self.config.translation_alignatt_source_lcp_stability
                ),
                "source_lcp_append_slack_units": int(
                    self.config.translation_alignatt_source_lcp_append_slack_units
                ),
            }
        )
        return MTBackendResult(
            ...
            accepted_generated_token_ids=previous_state.accepted_generated_token_ids,
    ...
    @staticmethod
    def _metadata_source_unit_indices(
        metadata: dict[str, Any] | None,
    ) -> tuple[int | None, ...] | None:
        if not metadata or "aligned_source_unit_indices" not in metadata:
            return None
        values = metadata.get("aligned_source_unit_indices")
    ...
    def source_bounded_accepted_prefill(
        self,
        *,
        final_source_unit_count: int,
    ) -> str:
        previous = self.state.partial_translation
        if not previous.accepted_target.strip():
            return ""
        ...
        if aligned_source_unit_indices is None:
            return previous.accepted_target
        bounded_generated_ids = source_bounded_prefill_generated_token_ids(
            accepted_generated_token_ids=previous.accepted_generated_token_ids,
            aligned_source_unit_indices=aligned_source_unit_indices,
            final_source_unit_count=final_source_unit_count,
        )
        if bounded_generated_ids == previous.accepted_generated_token_ids:
            return previous.accepted_target
        if not bounded_generated_ids:
            return ""
        ...
        bounded_text = mt_backend.decode_candidate_text(
            generated_ids=bounded_generated_ids,
            assistant_prefill="",
            variant=variant,
            is_partial=True,
        ).strip()
        if not bounded_text:
            return ""
        if previous.accepted_target.startswith(bounded_text):
            return bounded_text
        return ""

    def _decode_generated_target_ids(self, generated_ids: Sequence[int]) -> str:
        ...
        return mt_backend.decode_candidate_text(
            generated_ids=tuple(int(token_id) for token_id in generated_ids),
            assistant_prefill="",
            variant=variant,
            is_partial=True,
        ).strip()

    def _encode_semantic_target_ids(self, text: str) -> tuple[int, ...]:
        if not text.strip():
            return ()
        mt_backend = self.session.bundle.ensure_mt_backend()
        if not hasattr(mt_backend, "encode_semantic_target_token_ids"):
            return ()
        return tuple(int(token_id) for token_id in mt_backend.encode_semantic_target_token_ids(text))

    def commit_source_bounded_partial_acceptance(
        self,
        *,
        final_source_frontier: SourceAccessibilityFrontier,
        ...
        if (
            not previous.accepted_target.strip()
            or not previous.accepted_generated_token_ids
            or aligned_source_unit_indices is None
        ):
            return None

        final_generated_ids, residual_generated_ids = (
            split_source_bounded_prefill_generated_token_ids(
                accepted_generated_token_ids=previous.accepted_generated_token_ids,
                aligned_source_unit_indices=aligned_source_unit_indices,
                final_source_unit_count=final_source_frontier.accessible_unit_count,
            )
        )
        final_text = self._decode_generated_target_ids(final_generated_ids)
        if final_text and previous.accepted_target.startswith(final_text):
            residual_text = previous.accepted_target[len(final_text) :]
        else:
            residual_text = self._decode_generated_target_ids(residual_generated_ids)
        final_token_ids = self._encode_semantic_target_ids(final_text)
        residual_token_ids = self._encode_semantic_target_ids(residual_text)

        metadata = dict(previous.last_alignatt_metadata or {})
        metadata.update(
            {
                "source_bounded_committed_prefill": True,
                "source_bounded_final_source_unit_count": (
                    final_source_frontier.accessible_unit_count
                ),
                "source_bounded_final_token_count": len(final_generated_ids),
                "source_bounded_residual_token_count": len(residual_generated_ids),
            }
        )
        self.state.utt_translations.append(final_text)

        if residual_text.strip():
            rebased_source = self.normalize_source_text(
                self.session.current_live_asr_tail_text(),
                is_final=False,
            )
            rebased_frontier = self.build_source_frontier(
                rebased_source,
                is_final=False,
            )
            residual_unit_indices: list[int | None] = []
            source_unit_offset = int(final_source_frontier.accessible_unit_count)
            for source_unit_index in aligned_source_unit_indices[
                len(final_generated_ids) : len(final_generated_ids) + len(residual_generated_ids)
            ]:
                if source_unit_index is None:
                    residual_unit_indices.append(None)
                else:
                    residual_unit_indices.append(int(source_unit_index) - source_unit_offset)
            residual_metadata = dict(metadata)
            residual_metadata["aligned_source_unit_indices"] = residual_unit_indices
            residual_metadata["source_rebased_after_committed_split"] = True
            self.state.partial_translation = PartialTranslationState(
                source_prefix=rebased_source.text,
                accepted_target=residual_text.strip(),
                accepted_token_ids=residual_token_ids,
                accepted_generated_token_ids=residual_generated_ids,
                source_accessible_unit_count=rebased_frontier.accessible_unit_count,
                source_total_unit_count=len(rebased_frontier.units),
                last_accept_audio_seconds=previous.last_accept_audio_seconds,
                last_mt_audio_seconds=previous.last_mt_audio_seconds,
                last_alignatt_metadata=residual_metadata,
        ...
        return MTBackendResult(
            ...
            acceptance_text=final_text,
            draft_generated_token_ids=final_generated_ids,
            accepted_generated_token_ids=final_generated_ids,
            draft_token_ids=final_token_ids,
            accepted_token_ids=final_token_ids,
    ...
    def update_partial_state(
        ...
        ) = self.derive_monotone_acceptance(
    ...
    def sync_committed_translations(self) -> MTBackendResult | None:
        ...
        while len(self.state.utt_translations) < len(self.state.utt_sources):
            ...
            last_result = self.session.translate_with_mt(
                ...
                source_frontier=final_source_frontier,
...
class CascadeSession:
    ...
    def _transcribe_non_streaming(
        ...
        *,
        ...
        """
        asr_context = committed_transcript_tail(
            self.state.utt_sources[1:],
            max_words=int(getattr(self.config, "asr_context_committed_words", 0)),
        )
        try:
    ...
    def _try_commit_punctuation_lcp(
        ...
        *,
        ...
        min_commit_words = int(self.config.asr_punctuation_min_commit_words)
        if (
            min_commit_words > 0
            and len(remove_punctuation(committed_text).split()) < min_commit_words
        ):
            return None
        ...
        *,
        ...
        return InferenceArtifacts(
            ...
            runtime_config={
                ...
                "translation_alignatt_source_lcp_stability": self.config.translation_alignatt_source_lcp_stability,
                "translation_alignatt_source_lcp_append_slack_units": self.config.translation_alignatt_source_lcp_append_slack_units,
                ...
                "translation_alignatt_source_frontier_action": self.config.translation_alignatt_source_frontier_action,
                "translation_alignatt_max_inaccessible_source_mass": self.config.translation_alignatt_max_inaccessible_source_mass,
                "translation_alignatt_max_non_source_prompt_mass": self.config.translation_alignatt_max_non_source_prompt_mass,
                ...
                "translation_alignatt_min_accepted_accessible_source_mass": self.config.translation_alignatt_min_accepted_accessible_source_mass,
                "translation_alignatt_accepted_accessible_source_mass_recent_units": self.config.translation_alignatt_accepted_accessible_source_mass_recent_units,
                "translation_alignatt_min_accessible_source_units": self.config.translation_alignatt_min_accessible_source_units,
                "translation_alignatt_min_accessible_source_units_mode": self.config.translation_alignatt_min_accessible_source_units_mode,
                "translation_alignatt_hold_back_target_units": self.config.translation_alignatt_hold_back_target_units,
                "translation_alignatt_min_emit_target_units": self.config.translation_alignatt_min_emit_target_units,
                "translation_alignatt_max_source_regression": self.config.translation_alignatt_max_source_regression,
                "translation_alignatt_source_regression_min_source_mass": self.config.translation_alignatt_source_regression_min_source_mass,
                "translation_alignatt_source_regression_min_inaccessible_mass": self.config.translation_alignatt_source_regression_min_inaccessible_mass,
                "translation_alignatt_source_regression_recent_tokens": self.config.translation_alignatt_source_regression_recent_tokens,
                "translation_alignatt_source_regression_reference_mode": self.config.translation_alignatt_source_regression_reference_mode,
                "translation_alignatt_source_regression_activation_mode": self.config.translation_alignatt_source_regression_activation_mode,
                "translation_alignatt_source_regression_activation_slack_tokens": self.config.translation_alignatt_source_regression_activation_slack_tokens,
                "translation_alignatt_source_regression_patience_tokens": self.config.translation_alignatt_source_regression_patience_tokens,
                "translation_alignatt_source_regression_action": self.config.translation_alignatt_source_regression_action,
                "translation_alignatt_token_argmax_frontier_gate": self.config.translation_alignatt_token_argmax_frontier_gate,
                "translation_alignatt_token_argmax_min_source_mass": self.config.translation_alignatt_token_argmax_min_source_mass,
                "translation_alignatt_token_argmax_frontier_margin": self.config.translation_alignatt_token_argmax_frontier_margin,
                "translation_alignatt_token_argmax_frontier_patience_tokens": self.config.translation_alignatt_token_argmax_frontier_patience_tokens,
                "translation_alignatt_source_lookback_holdback": self.config.translation_alignatt_source_lookback_holdback,
                "translation_alignatt_source_lookback_units": self.config.translation_alignatt_source_lookback_units,
                "translation_alignatt_source_lookback_min_source_mass": self.config.translation_alignatt_source_lookback_min_source_mass,
                "translation_alignatt_source_lookback_min_source_position": self.config.translation_alignatt_source_lookback_min_source_position,
                "translation_alignatt_defer_low_source_terminal_punctuation": self.config.translation_alignatt_defer_low_source_terminal_punctuation,
                "translation_alignatt_terminal_punctuation_min_source_mass": self.config.translation_alignatt_terminal_punctuation_min_source_mass,
                ...
                "milmmt_prompt_mode": self.config.milmmt_prompt_mode,
                "milmmt_temperature": self.config.milmmt_temperature,
                "milmmt_top_p": self.config.milmmt_top_p,
                "milmmt_top_k": self.config.milmmt_top_k,
                "milmmt_repetition_penalty": self.config.milmmt_repetition_penalty,
