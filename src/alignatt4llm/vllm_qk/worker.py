...
Generalizes ``alignatt4llm.mt.gemma_vllm_worker.GemmaVLLMMTWorker``: the entire
warmup/compile-deferral lifecycle is model-agnostic, so a new model's worker is
just a subclass that sets ``spec``. The install/stub/configure steps are driven
by that spec; prepare/fetch/integrity reuse the proven Gemma helpers (they
operate on the per-layer observer buffers, which are identical across models).

Imports ``vllm`` at module load, like the Gemma worker, so it is only importable
inside a vLLM worker process (GPU).
"""
...
from alignatt4llm.vllm_qk.patch import (
    configure_mt_qk_observer_on_model,
    install_global_attention_mt_patch,
...
class BaseQKObserverWorker(VLLMGPUWorker):
    """Single-GPU worker that defers observer-aware warmup until request prep.

    Subclasses set the class attribute ``spec`` to a :class:`VLLMAttentionSpec`.
