...
def _ensure_transformers_generic_compat() -> None:
    from transformers.utils import generic as transformers_generic

    if hasattr(transformers_generic, "merge_with_config_defaults"):
        return
    check_model_inputs = getattr(transformers_generic, "check_model_inputs", None)
    if not callable(check_model_inputs):
        raise ImportError(
            "transformers.utils.generic lacks both merge_with_config_defaults "
            "and check_model_inputs"
        )
    transformers_generic.merge_with_config_defaults = check_model_inputs()
